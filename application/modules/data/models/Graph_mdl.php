...
public function dim2Graph($kpi,$dimension1) {
       ...
       $query = $this->db->query("SELECT  target_value,CONCAT( period,'-',period_year) as period,cal_value,dimension1,dimension2,dimension2_key FROM `report_trend_dimension2` WHERE trim(financial_year)='$this->financial_year' and kpi_id='$kpi' $filter order by period_year ASC, CHAR_LENGTH(period) ASC, period ASC")->result();
       $row_data = [];
...
public function dim3Graph($kpi,$dimension2) {
       $datas = array();
       $dimesnions = array();
       $periods = array();
       ...
       $query = $this->db->query("SELECT  target_value,CONCAT( period,'-',period_year) as period,cal_value,dimension2,dimension2_key,dimension3,dimension3_key FROM `report_trend_dimension3` WHERE trim(financial_year)='$this->financial_year' and kpi_id='$kpi' $filter order by period_year ASC, CHAR_LENGTH(period) ASC, period ASC")->result();
       $row_data = [];
