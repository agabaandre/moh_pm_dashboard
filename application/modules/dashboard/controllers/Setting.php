...
'Pacific/Guam' => 'Pacific/Guam',
	...
	public function create()
	{
		...
		$data['setting'] = (object)$postData = [
			...
			'slider_timer' => $this->input->post('slider_timer'),
			'footer_text' => $this->input->post('footer_text', true) ,
			'gauge_config' => $this->input->post('gauge_config', true),
			'font_awesome' => $this->input->post('font_awesome', true), 
			'kpi_rows' => $this->input->post('kpi_rows'),
	...
	function financialYear($period=FALSE){
		if(empty($period)){
		$period =0;
		}
		if(date('m') >= 06) {
		$d = date('Y-m-d', strtotime('+ years')); echo date('Y') .'-'.date('y', strtotime($d)); }

		else { 

		$d = date('Y-m-d', strtotime('-1 years'));

		$previous=date('Y', strtotime($d));
		$current = date('Y');


	return  strval($previous-$period).'-'.strval($current-$period);
