...
public function gaugeData($kpi){
    ...
    $query= $this->db->query("SELECT MAX(period) , CONCAT(period,'/',period_year) as cp, CONCAT(previous_period,'/',previousperiod_year) as pp, t.* from report_kpi_summary t WHERE  trim(kpi_id)='$kpi' GROUP BY period_year");
