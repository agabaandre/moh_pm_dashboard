        ...
        <ul class="treeview-menu">
            ...
            <?php if (count($outComeKpis) > 0): ?>
                ...
                <li class="treeview <?php echo ($outComeActive) ? 'active' : ''; ?>">
                    <a href="#" target="_self" onclick="openUrl('<?php echo $outComeUrl; ?>');">
                        <span>Outcome</span>
                        <ul class="treeview-menu">
                            <?php foreach ($outComeKpis as $kpi): ?>
                                <li data-toggle="tooltip" data-placement="right" title="<?php echo $kpi->short_name; ?>"
                                    style="z-index:1000;">
                                    <a href="<?php echo base_url() . 'data/kpiData/' . $kpi->kpi_id . '/' . $subject->id; ?>"
                                        class="text-truncate"><?php echo $kpi->short_name; ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </a>
            ...
            if (count($outPutKpis) > 0): ?>
                ...
                <li class="treeview <?php echo ($outPutActive) ? 'active' : ''; ?>">
                    <a href="#" title="Output KPIs" target="_self" onclick="openUrl('<?php echo $outPutUrl; ?>');">
                        <span>Output</span>
                        <ul class="treeview-menu">
                            <?php foreach ($outPutKpis as $kpi): ?>
                                <li data-toggle="tooltip" data-placement="right" title="<?php echo $kpi->short_name; ?>"
                                    style="z-index:1000;"><a
                                        href="<?php echo base_url() . 'data/kpiData/' . $kpi->kpi_id . '/' . $subject->id; ?>"
